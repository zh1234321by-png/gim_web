...
export default function ProductSearch({ condensed = false }: { condensed?: boolean }) {
  ...
  const runSearch = () => {
    ...
    let name = "";
    if (kind === "gim-long") name = `XAN0OPSRAP_${year}${doy}0000_01D_01H_GIM.INX.gz`;
    ...
    setResults([{ name, cadence: kind === "dcb" ? "1天" : "1小时", status: mode === "archive" ? "可访问产品目录" : "实时产品", url: `${base}/${year}/${doy}/${name}` }]);
  };
  ...
  return (
    <section className={`product-search ${condensed ? "condensed-search" : ""}`} aria-label="电离层产品检索">
      <div className="search-tabs" role="tablist" aria-label="产品时效">
        <button type="button" className={mode === "archive" ? "active" : ""} onClick={() => setMode("archive")}>事后产品</button>
        <button type="button" className={mode === "realtime" ? "active" : ""} onClick={() => setMode("realtime")}>
          实时产品 <span className="mini-badge">即将上线</span>
        </button>
      </div>
      ...
      {!searched && <p className="search-note">当前公开目录：2024 年 XUST GIM / DCB 事后产品。示例日期已设为首个可用历元。</p>}
    </section>
  );
}
