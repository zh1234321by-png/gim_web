...
const inputArg = args[0] || '../public/realtime/XAN1OPSRTS_20262140000_01D_05M_GIM.INX.gz';
const outputArg = args[1] || '../public/realtime/demo.json';
...
function parseRowHeader(line) {
  ...
  if (tokens.length >= 4 && tokens[0].includes('.') && tokens[1].includes('.') && tokens[2].includes('.') && tokens[3].includes('.')) {
    return {
      lat: Number(tokens[0]),
...
while (idx < lines.length) {
  ...
  frames.push({
    ...
    source: {
      ...
      mountpoint: path.basename(inputPath).split('_')[0],
